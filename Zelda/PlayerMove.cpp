...
void PlayerMove::Update(float deltaTime) {
	if (!((mPlayer->GetGame()->mEnemies).size() == 0)){
		if (mPlayer->GetGame()->mEnemies.count(mGame->currRoom)) {
			if (!(mPlayer->GetGame()->mEnemies.at(mPlayer->GetGame()->currRoom).size() == 0)) {
				if (!mPlayer->GetGame()->mEnemies.at(mPlayer->GetGame()->currRoom).empty()) {
					std::vector<Actor*> enemiesInRoom = mPlayer->GetGame()->mEnemies.at(mPlayer->GetGame()->currRoom);
					for (auto e : enemiesInRoom) {
						if (e->GetState() != ActorState::Destroy) {
